import pandas as pd
import numpy as np
from config import MONTH_DICT, QUARTER_DICT, BRANCH_METRIC_GROUPS, GROUP_WEIGHT_VARS
import math
        ...
        if segment == "Branch":
            try:
                df["Branch"] = df["Branch"].map(
                    {1: "Dubai", 2: "Sharjah", 3: "Abu Dhabi"}
                )
                df["NATIONALITY"] = df["NATIONALITY"].map(
                    {1: "Emarati", 2: "Non-Emarati"}
                )
                df["Q1_1"] = df["Q1_1"].map(
                    {
                        1: "Social Media Lead",
                        2: "Website Visit Lead",
                        3: "Call centre Lead",
                        4: "Walkin Customer",
                    }
                )
...
def get_available_quarters(df):
    ...
    return quarters if quarters else []
...
def load_branch_data(user_id=None):
    ...
    AVAILABLE_QUARTERS = get_available_quarters(branch_df)
    ...
    processed_data = prepare_dashboard_data(
        branch_df, AVAILABLE_MONTHS, AVAILABLE_QUARTERS, "branch"
    )
...
def load_contact_centre_data(user_id=None):
    ...
    contact_centre_df = load_data(
        "CHERY_CONTACT_CENTRE_EVAL_DASH CSV.csv", "Contact Centre"
    )
    ...
    AVAILABLE_QUARTERS = get_available_quarters(contact_centre_df)
    ...
    # Prepare dashboard data
    processed_data = prepare_dashboard_data(
        contact_centre_df, AVAILABLE_MONTHS, AVAILABLE_QUARTERS, "contact-centre"
    )
...
def load_website_data(user_id=None):
    ...
    AVAILABLE_QUARTERS = get_available_quarters(website_df)
    ...
    # Prepare dashboard data
    processed_data = prepare_dashboard_data(
        website_df, AVAILABLE_MONTHS, AVAILABLE_QUARTERS, "website"
    )
...
def load_combined_contact_centre_data(user_id=None):
    ...
    AVAILABLE_QUARTERS = get_available_quarters(cc_combined_df)
    ...
    # Prepare dashboard data
    processed_data = prepare_dashboard_data(
        cc_combined_df, AVAILABLE_MONTHS, AVAILABLE_QUARTERS, "combined-contact-centre"
    )
...
def load_social_media_data(user_id=None):
    ...
    AVAILABLE_QUARTERS = get_available_quarters(social_media_df)
    ...
    # Prepare dashboard data
    processed_data = prepare_dashboard_data(
        social_media_df, AVAILABLE_MONTHS, AVAILABLE_QUARTERS, "social-media"
    )
...
def prepare_dashboard_data(
    df, AVAILABLE_MONTHS=None, segment=None, AVAILABLE_QUARTERS=None
):
    ...
    for group_name, metrics in metric_groups.items():
        ...
        if weight_var and weight_var in df.columns:
            ...
            group_score = (
                int(round(group_score, 0))
                if not math.isnan(group_score)
                else group_score
            )
            ...
            for month in AVAILABLE_MONTHS:
                ...
                if not month_df.empty and weight_var in month_df.columns:
                    ...
                    monthly_scores[month] = (
                        int(round(month_score, 0))
                        if not math.isnan(month_score)
                        else month_score
                    )
            # Calculate monthly scores for the group
            quarterly_scores = {}
            for q in AVAILABLE_QUARTERS:
                ...
                if not q_df.empty and weight_var in q_df.columns:
                    q_score = calculate_metric_score(q_df, weight_var)
                    # monthly_scores[month] = int(round(month_score, 0))
                    quarterly_scores[q] = (
                        int(round(q_score, 0)) if not math.isnan(q_score) else q_score
                    )
        ...
        for metric_id, metric_name in metrics.items():
            if metric_id in df.columns:
                ...
                for q in AVAILABLE_QUARTERS:
                    q_df = df[df["QUARTER"] == q]
                    if not q_df.empty:
                        q_score = calculate_metric_score(q_df, metric_id)
                        # monthly_scores[month] = round(month_score, 0)
                        quarterly_scores[q] = (
                            int(round(q_score, 0))
                            if not math.isnan(q_score)
                            else q_score
                        )
