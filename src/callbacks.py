...
def register_callbacks(app, branch_df, contact_center_df, processed_data, branch_available_months, contact_center_available_months):
    ...
    def render_segment_content(active_tab):
        if active_tab:
            ...
            if active_tab == 'branch':
                ...
                # For other segments, use default values
                branches = ['Overall']
                appointment_types = ['Overall']
                nationalities = ['Overall']
                available_months = contact_center_available_months if active_tab == 'contact-center' else []
    ...
    # Similar callbacks for other segments
    for segment in ['website', 'social-media']:
        @app.callback(
            Output(f"{segment}-groups-container", "children"),
            Output(f"{segment}-visit-count", "children"),
            [Input(f"{segment}-branch-filter", "value"),
             Input(f"{segment}-appointment-filter", "value"),
             Input(f"{segment}-month-filter", "value"),
             Input(f"{segment}-nationality-filter", "value")]
        )
        def update_segment_content(branch, appointment_type, month, nationality, segment=segment):
