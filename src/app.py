...
from data_loader import (
    load_branch_data,
    load_contact_centre_data,
    load_website_data,
    load_social_media_data,
    load_combined_contact_centre_data,
)
            ...
            [
                html.Div(
                    [
                        ...
                        html.Div(
                            id="login-error", className="login-error", children=""
                        ),
                    ],
...
def load_all_data():
    ...
    (
        branch_df,
        branch_processed_data,
        branch_available_months,
        branch_available_quarters,
        branch_branches,
        branch_appointment_types,
        branch_nationalities,
        branch_sc_names,
    ) = load_branch_data()
    (
        contact_centre_df,
        contact_centre_processed_data,
        contact_centre_available_months,
        contact_centre_available_quarters,
        _,
        _,
        _,
        contact_centre_sc_names,
    ) = load_contact_centre_data()
    (
        website_df,
        website_processed_data,
        website_available_months,
        contact_centre_available_quarters,
        _,
        _,
        _,
    ...
    (
        ...
        social_media_available_months,
        contact_centre_available_quarters,
        _,
        _,
        _,
        social_media_sc_names,
    ) = load_social_media_data()
    (
        combined_contact_centre_df,
        combined_contact_centre_processed_data,
        combined_contact_centre_available_months,
        contact_centre_available_quarters,
        _,
        _,
        _,
        combined_contact_centre_sc_names,
    ) = load_combined_contact_centre_data()
    ...
    for user_id in VALID_CODES.values():
        if user_id == "Admin":
            ...
            filtered_sc_names = (
                sorted(all_sc_names) if all_sc_names else branch_sc_names.copy()
            )

        else:
            # Filter data for specific branch/location
            filtered_branch_df = branch_df[
                branch_df["Branch"].str.contains(user_id, case=False)
            ]
            filtered_branches = [
                b for b in branch_branches if user_id.lower() in b.lower()
            ]
        ...
        [
            ...
            dbc.Row(
                [
                    dbc.Col(
                        [
                            html.Img(
                                src=app.get_asset_url("logo5.png"), className="logo"
                            )
                        ],
                        width=2,
                    ),
                    ...
                    dbc.Col(
                        [
                            html.Div(
                                html.Img(
                                    src=app.get_asset_url("CYN_Logo2.png"),
            ...
            dbc.Tabs(
                [
                    ...
                    dbc.Tab(
                        label=SEGMENTS["combined-contact-centre"],
                        tab_id="combined-contact-centre",
                    ),
                ],
                id="segment-tabs",
                active_tab="branch",
                    ...
                    dbc.Col(
                        dbc.RadioItems(
                            ...
                            options=[
                                ...
                                {"label": "Quarterly Trend", "value": "quarterly"},
                            ],
                            value="monthly",
                            inline=True,
                            className="mb-1",
